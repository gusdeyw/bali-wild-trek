...
if (isset($_POST['pengguna'])) {
    ...
    if (
        ($pengguna == "admin" && $password == "admin1234") ||
        ($pengguna == "gusde" && $password == "WEBgusde123!") ||
        ($pengguna == "arta" && $password == "WEBarta123!")
    ) {
        $_SESSION['su'] = "1";
        header("Location: index.php");
    }
...
<head>
    ...
    <title>Login - Bali Wild Trek</title>
                ...
                <form action="login.php" method="POST" class="w-full">
                    <div class="flex items-center justify-center p-6 sm:p-12 w-full">
                        ...
                        <div class="w-full">
                            ...
                            <button type="submit"
                                class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">Log
                                in</button>
